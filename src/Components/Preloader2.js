...
const ClassicPreloader = () => {
    ...
    useEffect(()=>{
        ...
        for(let x=0; x<svgIndex.length; x++ ){
            ...
            gsap.to('path',{stroke:'#fff',strokeDashoffset:0,duration:2})
        }  
        
        gsap.to('.Tstroke1',{scale:0.9,delay:2,rotate:'-60deg',x:-50,y:80})
        gsap.to('.Tstroke2',{scale:0.5,delay:2,rotate:'0deg',x:-70,y:80})
        gsap.to('.Qstroke',{scale:0.5,delay:2,rotate:'-25deg',x:30,y:125})
        gsap.to('.tail',{fill:"#bd0000",delay:2,rotate:'45deg',x:-65,y:130})
        gsap.to('.tail',{fill:"#FFD600",delay:2.8 })

        gsap.timeline()    
        .to('.shadow',{delay:3,opacity:1})
        .to('.stars',{opacity:1})
        .to('.preloaderInner',{delay:0.4,y:-650,duration:1})
        // .to('.shadow', {opacity:0},'-=0.5')
        .to('.shadow', {scaleX:3,duration:1},'-=1')
        .to('.shadow', {scale:1300,duration:.7})
